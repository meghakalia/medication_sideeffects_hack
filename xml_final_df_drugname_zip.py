...
def search_reports_by_active_ingredient(zip_path,xmlname, active_ingredient):
    with zipfile.ZipFile(zip_path, 'r') as zip_file:
    # Check if the XML file exists in the ZIP
        if xmlname in zip_file.namelist():
        # Read the XML file
            with zip_file.open(xmlname) as xml_file:
...
matching_reports, reaction_counts = search_reports_by_active_ingredient(xml_file_path,xml_name, active_ingredient)
